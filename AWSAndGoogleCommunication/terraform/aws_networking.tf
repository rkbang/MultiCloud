...
resource "aws_subnet" "rkb-aws-subnet1" {
  vpc_id     = aws_vpc.rkb-aws-vpc.id
  cidr_block = var.aws_subnet1_cidr

  tags = {
    Name = "rkb-aws-subnet1"
  }
}
...
resource "aws_default_route_table" "rkb-aws-vpc" {
  default_route_table_id = aws_vpc.rkb-aws-vpc.default_route_table_id
  route {
