...
resource "google_compute_firewall" "gcp-allow-icmp" {
  name    = "${google_compute_network.rkb-gcp-network.name}-gcp-allow-icmp"
  network = google_compute_network.rkb-gcp-network.name
...
resource "google_compute_firewall" "gcp-allow-ssh" {
  name    = "${google_compute_network.rkb-gcp-network.name}-gcp-allow-ssh"
  network = google_compute_network.rkb-gcp-network.name
...
resource "google_compute_firewall" "gcp-allow-vpn" {
  name    = "${google_compute_network.rkb-gcp-network.name}-gcp-allow-vpn"
  network = google_compute_network.rkb-gcp-network.name
...
resource "google_compute_firewall" "gcp-allow-internet" {
  name    = "${google_compute_network.rkb-gcp-network.name}-gcp-allow-internet"
  network = google_compute_network.rkb-gcp-network.name
